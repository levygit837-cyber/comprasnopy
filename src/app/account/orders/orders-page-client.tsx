...
export function OrdersPageClient({ empty, children }: OrdersPageClientProps & { children?: React.ReactNode }) {
  ...
  if (empty) {
    return (
      <>
        <h1 className="font-sans text-2xl md:text-3xl font-semibold text-[var(--brand-primary)] tracking-tight mb-4">
          {t("ordersTitle")}
        </h1>
        <div className="theme-aware bg-[var(--bg-card)] border border-[var(--bg-border)] rounded-2xl p-6 text-center">
          ...
          <Link
            href="/products"
            className="inline-flex bg-[var(--brand-action)] text-white font-bold px-5 py-2 rounded-full hover:bg-[var(--brand-action-hover)] transition-colors text-xs"
          >
